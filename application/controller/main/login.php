...
class controller_main_login extends controller_main_base
{
	...
	public function run()
	{
		...
		{
			$_SESSION['notice']['default'] = "<span class=\"red\"><i class=\"icon-exclamation-sign icon-2x\"></i></span>  Bitte Benutzername und Passwort eingeben!";
		...
		else
		{
			...
			if(is_null($admin))
			{
				$_SESSION['notice']['default'] = "<span class=\"red\"><i class=\"icon-exclamation-sign icon-2x\"></i></span>  Der Benutzername ist unbekannt!";
				Header("Location: " . cfg::$web_root);
				exit();
			}
			elseif($password != $admin->password )
			{
				$_SESSION['notice']['default'] = "<span class=\"red\"><i class=\"icon-exclamation-sign icon-2x\"></i></span>  Benutzername und Passwort stimmen nicht überein!";
				Header("Location: " . cfg::$web_root);
				exit();
			}
			elseif($admin->active == 0)
			{
				$_SESSION['notice']['default'] = "<span class=\"red\"><i class=\"icon-exclamation-sign icon-2x\"></i></span>  Login nicht gestattet!";
				Header("Location: " . cfg::$web_root);
				exit();
			}
			else
