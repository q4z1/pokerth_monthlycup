 ...
 *
 */
class mixed_upload2025 extends mixed_base
{
	...
	public static function get_hall_of_fame()
	{
		...
		$sql = "
      SELECT sum(u.points) as points, u.playername, p.awards, p.avatar, p.avatar_mime FROM upload2025 u
      LEFT JOIN `player2025` p USING (playername)
    ...
    $month = $db->escape($month);
		$sql = "
      SELECT u.playername, u.position, p.avatar, p.avatar_mime FROM upload2025 u
      LEFT JOIN `player2025` p ON(u.playername = p.playername)
      WHERE u.month = $month
      AND table_ = 'gold'
      AND position < 4
    ...
    $month = $db->escape($month);
		$sql = "
      SELECT u.playername, u.position FROM upload2025 u
      WHERE u.month = $month
      AND table_ = 'gold'
      AND position < 4
      ORDER BY u.position ASC;
	...
	public static function get_general_ranking(){
		$db = database::get_instance();
		$sql = "
      SELECT sum(points) as points, playername FROM upload2025
			WHERE TRUE
	...
	public static function get_points_by_player_month($player, $month){
		...
		$month = $db->escape($month);
		$sql = "
      SELECT sum(points) as points FROM upload2025
			WHERE 
