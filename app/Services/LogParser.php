...
/**
 * Reads a PokerTH game log database (SQLite) and returns the game statistics.
 * Ported unchanged from the legacy lib/logparser.php – only the namespace,
 * the class name and the configurable log directory differ.
 */
class LogParser
{
    private $pdo = null;
	...
	{
		return rtrim(config("mcup.log_dir"), "/");
	}
	...
	public function process_log_file($pdb, $id){
		if(!file_exists($this->logDir() . "/$pdb")){
			return ['status' => 'error', 'data' => "File $pdb not found."];
		}
		if(is_null($this->game_id) && !is_null($id)) $this->game_id = $id;
		try{
			$this->pdo = new PDO("sqlite:" . $this->logDir() . "/$pdb");
		}catch(Exception $e){
    ...
    private function fetch_database($url){
		...
		$this->game_id = isset($query['game_id']) ? (int) $query['game_id'] : null;
		$pdb = $query['pdb'] ?? '';
		// only ever look at a plain file name inside the configured log directory
		$pdb = preg_replace('/[^A-Za-z0-9_-]/', '', $pdb);
		return $pdb === '' ? '' : $pdb . '.pdb';
	}
